...
export default function Beneficiaries({ user }) {
 ...
 const photo=async e=>{const f=e.target.files?.[0];if(!f)return;if(!f.type.startsWith('image/'))return alert('Моля, избери изображение.');try{setSaving(true);setForm(x=>({...x,photo:await compressImage(f)}))}catch(err){alert('Грешка при снимката: '+err.message)}finally{setSaving(false)}};
