...
export default function Beneficiaries({ user }) {
 ...
 const [form,setForm]=useState(emptyForm),[photoInputKey,setPhotoInputKey]=useState(0),[history,setHistory]=useState([]),[historyLoading,setHistoryLoading]=useState(false);
 ...
 useEffect(()=>{load()},[]);
 useEffect(()=>{
   if(!selectedProfile || !isAdmin){setHistory([]);return;}
   let cancelled=false;
   (async()=>{try{setHistoryLoading(true);const s=await get(ref(database,'auditLogs'));const all=s.exists()?Object.values(s.val()):[];const rows=all.filter(x=>x.module==='Бенефициенти'&&x.recordId===selectedProfile.firebaseKey).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0)).slice(0,30);if(!cancelled)setHistory(rows)}catch(e){if(!cancelled)setHistory([])}finally{if(!cancelled)setHistoryLoading(false)}})();
 ...
 const save=async e=>{e.preventDefault();if(!form.firstName.trim()||!form.lastName.trim())return alert('Собствено име и фамилия са задължителни.');if(form.identifier.trim().length>100)return alert('Идентификаторът е прекалено дълъг.');try{setSaving(true);const id=editing?items[editing]?.id:nextId(items);const now=Date.now();const data={id:Number(id),firstName:form.firstName.trim(),middleName:form.middleName.trim(),lastName:form.lastName.trim(),gender:form.gender,birthDate:form.birthDate,identifier:form.identifier.trim(),status:form.status,active:form.status==='active',archived:form.status==='archived',photo:form.photo||'',updatedBy:user?.uid||'unknown',updatedAt:now};
 ...
 else {const r=push(ref(database,'beneficiaries'));const x={...data,createdBy:user?.uid||'unknown',createdAt:now};await update(r,x);setItems(p=>({...p,[r.key]:x}));await logAudit({user,action:'Добавяне',module:'Бенефициенти',recordId:r.key,details:[x.firstName,x.lastName].filter(Boolean).join(' ')})}
 reset();setView('list');
 }catch(err){alert('Грешка при записване: '+err.message)}finally{setSaving(false)}};
 const archive=async x=>{const restoring=x.status==='archived';const action=restoring?'възстановиш':'архивираш';if(!confirm('Сигурен ли си, че искаш да '+action+' бенефициент №'+x.id+'?'))return;try{setSaving(true);const now=Date.now();const data=restoring?{status:'active',active:true,archived:false,restoredBy:user?.uid||'unknown',restoredAt:now,updatedBy:user?.uid||'unknown',updatedAt:now}:{status:'archived',active:false,archived:true,archivedBy:user?.uid||'unknown',archivedAt:now,updatedBy:user?.uid||'unknown',updatedAt:now};await update(ref(database,'beneficiaries/'+x.firebaseKey),data);await logAudit({user,action:restoring?'Възстановяване':'Архивиране',module:'Бенефициенти',recordId:x.firebaseKey,details:[x.firstName,x.lastName].filter(Boolean).join(' ')});setItems(p=>({...p,[x.firebaseKey]:{...p[x.firebaseKey],...data}}));if(selectedProfile?.firebaseKey===x.firebaseKey)setSelectedProfile(p=>({...p,...data}));}catch(e){alert('Грешка при промяна на статуса: '+e.message)}finally{setSaving(false)}};
 const hardDelete=async x=>{if(!isAdmin)return;if(!confirm('Това ще изтрие бенефициент №'+x.id+' окончателно. Продължаване?'))return;try{setSaving(true);await remove(ref(database,'beneficiaries/'+x.firebaseKey));await logAudit({user,action:'Окончателно изтриване',module:'Бенефициенти',recordId:x.firebaseKey,details:[x.firstName,x.lastName].filter(Boolean).join(' ')});setItems(p=>{const n={...p};delete n[x.firebaseKey];return n});setSelectedProfile(null)}catch(e){alert('Грешка при окончателно изтриване: '+e.message)}finally{setSaving(false)}};
